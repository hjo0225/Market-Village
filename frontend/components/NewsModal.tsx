...
interface Props {
  ...
  onSkip?: () => void;
  // T-286(사용자 리포트) — 이미 고른 날은 재선택 금지: 고른 카드만 밝게, 잠금 표시.
  chosenId?: string | null;
}

export default function NewsModal({ isOpen, onClose, news, onSelect, onSkip, chosenId }: Props) {
      ...
      <div className="relative w-full max-w-3xl flex flex-col items-center gap-6 animate-bump">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-extrabold text-white drop-shadow-md">
            {locked ? "오늘 아침, 이 뉴스를 골랐다" : "오늘, 어떤 뉴스가 눈에 들어왔나?"}
          </h2>
          <p className="text-sm text-white/70 mt-1">
            {locked ? "선택은 하루에 한 번 — 내일 아침 새 뉴스가 온다." : "고른 뉴스가 클론과 마을의 마음을 흔든다."}
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
          {news.map((n, i) => {
            ...
            const isChosen = n.id === chosenId;
            return (
              <button
                key={n.id}
                disabled={locked}
                onClick={locked ? undefined : () => { onSelect(n.id); onClose(); }}
                style={{ animationDelay: `${i * 130}ms` }}
                className={`${locked ? "" /* 등장 애니메이션의 opacity fill이 흐림을 덮음 */ : "animate-card-reveal"} group relative flex-1 min-w-[180px] rounded-2xl border-2 border-black
                  bg-white shadow-pixel-md p-5 text-center select-none
                  transition-all duration-150 ease-out
                  ${locked
                    ? isChosen
                      ? "ring-4 ring-pixel-grass"
                      : "opacity-35 grayscale"
                    : "cursor-pointer hover:-translate-y-1.5 hover:shadow-pixel-lg"}`}
              >
                {isChosen && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 text-[11px] font-extrabold bg-pixel-grass border-2 border-black rounded-full px-2 py-0.5">
                    ✓ 오늘의 선택
                  </span>
                )}
