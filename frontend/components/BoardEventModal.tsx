      ...
      <div className="flex items-center gap-2 mb-1.5">
        {post.portrait ? (
          <Image
            ...
            alt={post.author} width={28} height={28}
            className="rounded-full border border-black bg-pixel-grass/30"
            unoptimized
          />
        ) : (
          <span className="w-7 h-7 rounded-full border border-black bg-pixel-grass/40 flex items-center justify-center text-[14px]">🤖</span>
        )}
        <span className="text-sm font-extrabold">{post.author}</span>
        {/* T-246 — 이름 옆에 역할 부기(이름만으론 성격을 모르니). */}
        <span className="text-[12px] text-pixel-muted">· {post.author_role ?? "오늘"}</span>
        {/* T-260 — SNS다움: 올린 시각(결정론 파생) */}
        <span className="text-[11px] text-pixel-muted">· {seedNum(post.text, 50, 7) + 2}분 전</span>
