...
// §2.1 — 스테이블·현금은 "리스크 자산"이 아니라서 리스크 비중 계산에서 제외.
const RISK_CATEGORIES: Category[] = ["large_stable", "mid_alt", "meme"];
...
export default function PortfolioPanel({
  ...
  const riskTotal = RISK_CATEGORIES.reduce((s, c) => s + (holdings[c] ?? 0), 0);
  const riskSharePct = total > 0 ? Math.round((riskTotal / total) * 100) : 0;
  ...
  return (
    <PixelPanel tone="cloud" className={`p-4 ${className}`}>
      ...
      {/* §2.1 — 리스크 비중: 전체 자산 중 가격이 출렁이는 코인의 비율 */}
      <div className="flex items-center justify-between mb-3 text-[11px] text-pixel-muted">
        <Term term="리스크 비중" />
        <span className="font-bold tabular-nums">{riskSharePct}%</span>
      </div>
      <div className="flex flex-col gap-2">
        {CATEGORIES.map((cat: Category) => {
          ...
          const label = cat === "stable" ? <Term term="스테이블코인">스테이블</Term>
            : cat === "cash" ? <Term term="현금화">현금</Term>
            : CATEGORY_LABEL[cat];
          return (
            <div key={cat} className="flex flex-col gap-1">
              <div className="flex items-center gap-2 text-[11px]">
                <Icon className={`w-4 h-4 shrink-0 ${TEXT[cat]}`} aria-hidden />
                <span className="w-16 shrink-0 text-pixel-muted">{label}</span>
