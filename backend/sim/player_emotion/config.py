...
# T-51 (F4): 게시판 감정-소모 액션. 매수/매도/유지는 감정을 **현재값의 비율만큼 소모**하고,
# 그 소모량이 매매 규모의 원천이다(감정 비례). 이 플로어 미만이면 그 감정 기반 액션은
# 비활성 — "안 느끼는 감정으론 못 움직인다"(거울 강화). 잠정 튜닝치(플레이테스트 확정).
EMOTION_ACTION_FLOOR: float = 10.0
