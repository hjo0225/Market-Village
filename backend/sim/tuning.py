...
# §9.3 폭주 방지 — crowd_mood는 밤사이 중립(50)으로 이만큼 회귀한다.
# T-250(council F4, T-230 lite): 델타가 부호를 갖게 되면서(공포는 -, 탐욕은 +)
# 0.20으로 상향 — 최악 지속 +6/일에도 평형 50+6×(1-0.2)/0.2 = 74 < M2 75.
# M2는 평형으로 상시 발동 불가, 탐욕 이벤트 연속일에만 일시 돌파(의도된 광기 구간).
CROWD_MOOD_REVERT = 0.20
# T-250 — 게시판/LLM 군중 델타 클램프(부호 포함). PLAN-READY 4b: 감쇠 0.20 ·
# 클램프 ±6 · 평형 74(<M2 75) — 3칸 충족.
CROWD_DELTA_CLAMP = 6.0
...
# T-271 — 긴급 속보 사이렌(유저 트리거 외란, 심리 전용 — 가격·운명선 불가침 §7.1).
# 강도 계열: 뉴스 −6 > 사이렌 ±5 > 외란 ±2~4. 새 누적 게이지 없음(4b 비해당) —
# 효과 수령처는 기존 3칸 검증 완료(스탯 clamp 0~100, crowd 감쇠 0.20·평형 74<75).
# 에피소드성(기대 0.15회/일)이라 crowd 기대 기여 +0.75/일 → 평형 +3 이내. [TUNE]
SIREN_PROB_PCT = 15         # 발생 확률 %/일 — 30일 기대 4.5회
SIREN_STAT_DELTA = -5.0     # 관련 저항 스탯 델타
