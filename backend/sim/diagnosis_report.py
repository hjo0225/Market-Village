...
# T-66 — 마무리 문장은 실제 게임 일수(3일 압축/10일 정식)에 맞게 말한다.
# days_word(ending.py)가 "열흘"/"사흘" 표기의 단일 소스.
def _closing_line(total_days: int = 10) -> str:
    return (f"클론은 이 성향대로 {days_word(total_days)}을 산다. "
            "진짜 당신과 같은지는, 끝에 확인하자.")


def _summary(declared_type: str, total_days: int = 10) -> list[str]:
    lines = list(_SUMMARY_LINES.get(declared_type, _SUMMARY_LINES[DECLARED_TYPES[2]]))
    lines.append(_closing_line(total_days))
    return lines


def build_diagnosis(answers: dict, total_days: int = 10) -> dict:
    ...
    `total_days` — 요약 마무리 문장의 일수 표기용(생략 시 기존 10일 문구 그대로)."""
    disp = diagnose(answers)
    return {
        "declared_type": disp["declared_type"],
        "axes": _axes(disp),
        "contributions": _contributions(answers),
        "summary": _summary(disp["declared_type"], total_days),
    }
