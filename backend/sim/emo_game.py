...
TRADE_INTENSITY = 0.6
# T-63 (2안) — 클론 본능을 거스른 대가(불안). 성향과 다른 액션을 고르면 그날 캐스케이드에
# 이 델타가 얹힌다. 잠정 튜닝치(anxiety+4) — 과하면 감쇠0.15·클램프와 재검산.
INSTINCT_DEFY_DELTA: dict[str, float] = {"anxiety": 4.0}
...
class EmoGameRun:
    ...
    # T-63 (2안) — 클론 본능 거스름 집계. defied=본능과 다른 액션 선택 횟수, total=본능 있는
    # 게시판 결정 횟수. 엔딩 리포트 규율 점수(disposition_report)로 소비.
    discipline_tally: dict[str, int] = field(default_factory=lambda: {"defied": 0, "total": 0})
    ...
    def board(self) -> dict:
        ...
        return board_exposure.render_board(
            ...
            declared_type=(self.disposition or {}).get("declared_type"),   # T-63 — 본능 배지
        )
    ...
    def choose(self, choice_id: str, coin_target: str | None = None) -> None:
        ...
        # T-63 (2안) — 클론 본능 vs 실제 선택. 본능(2-1-2)을 거스르면 불안 비용 + 규율 집계.
        # 관찰오염 방지: declared_type만 참조(expected_bias/편향축은 비노출 유지).
        instinct = instinct_action((self.disposition or {}).get("declared_type", ""))
        if instinct is not None:
            self.discipline_tally["total"] = self.discipline_tally.get("total", 0) + 1
            if action != instinct:
                self.discipline_tally["defied"] = self.discipline_tally.get("defied", 0) + 1
                defied_state = apply_delta(state, INSTINCT_DEFY_DELTA)
                _push_step("instinct", "본능을 눌렀다", state, defied_state,
                           {"instinct": instinct, "action": action})
                state = defied_state
    ...
    def to_doc(self) -> dict:
        return {
            ...
            "discipline_tally": self.discipline_tally,       # T-63 — 본능 거스름 집계
    ...
    def from_doc(cls, doc: dict) -> "EmoGameRun":
        ...
        return cls(
            ...
            discipline_tally=dict(doc.get("discipline_tally") or {"defied": 0, "total": 0}),
